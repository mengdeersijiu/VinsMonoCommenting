...
//这里的result_delta_p、result_delta_v和result_delta_q就是相对于bk帧的预积分，
// 对应预计分类IntergrationBase类中delta_p、delta_v、delta_q三个全局变量，每来一帧新图像或者滑动一次窗口时就会新创建一个预积分，
// 创建时预积分中的delta_p、delta_v、delta_q中这三个量都设为零，然后在获得新的IMU值之后不断积分。
...
//这里的result_delta_p、result_delta_v和result_delta_q就是相对于bk帧的预积分，
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        //用一阶近似对中值积分得到的预积分测量值进行矫正，从而得到 更加精确的预积分测量值（bias修正的线性模型）
        // 对应IMU model中的\hat{\alpha},\hat{\beta},\hat{\gamma}
        Eigen::Quaterniond corrected_delta_q = delta_q * Utility::deltaQ(dq_dbg * dbg);
        Eigen::Vector3d corrected_delta_v = delta_v + dv_dba * dba + dv_dbg * dbg;
        Eigen::Vector3d corrected_delta_p = delta_p + dp_dba * dba + dp_dbg * dbg;

        // IMU项residual计算,传参是状态的估计值, 上面correct_delta_*是预积分值, 二者求'diff'得到residual.
